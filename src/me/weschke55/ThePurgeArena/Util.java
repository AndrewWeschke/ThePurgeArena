	...
	// ITEMSTACK
		...
		public static ItemStack parseItemStack(String s) {
			try {
				...
				System.err.println("[PurgeArena] Cannot parse ItemStack: " + s + " - Mabye this is the reason: " + e.toString());
				return null;
			}
		}
		...
		public static Location parseLocation(String s) {
			...
			try {
				...
				System.err.println("[PurgeArena] Cannot parse location from string: " + s);
			}
		...
		public static void debug(Object object) {
			if(debug) {
				System.out.println("[PurgeArena] [Debug] " + object.toString());
		...
		public static void checkForOutdatedArenaSaveFiles() {
			java.io.File f = new java.io.File("plugins/SurvivalGames/reset/");
			List<String> outdated = new ArrayList<>();
			if(f.exists() && f.isDirectory()) {
				for(String key : f.list()) {
					if(!key.endsWith(".map"))
						continue;
					SchematicFormat sf = SchematicFormat.getFormat("plugins/SurvivalGames/reset/" + key);
					if(sf == null) {
						outdated.add(key);
					}
				}
